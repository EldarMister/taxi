...
Для Nominatim включены исходящие IPv6-соединения в Railway и штатная репликация из `kyrgyzstan-updates` Geofabrik. `REPLICATION_UPDATE_INTERVAL=86400` задаёт размер суточного пакета изменений; при отсутствии новых данных сервис повторяет проверку через `REPLICATION_RECHECK_INTERVAL=900` секунд. Отдельный процесс перезапускает репликацию через 60 секунд после сбоя. После импорта права на каталог проекта выставляются явно: без этого первый запуск мог оставить каталог владельцу root, остановить репликацию и сохранить устаревшие адреса при работающем API. Свежесть проверяйте по `data_updated` в `/status?format=json`. Без исходящего IPv6 Railway не достигает Geofabrik, и Nominatim завершается при запуске; перед выключением IPv6 сначала отключите репликацию.
