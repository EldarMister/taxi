...
import { getDriverTrackingDiagnostics } from './native/driverTracking';
...
function TurnArrow({ modifier, type, arrived, color = 'white' }: { modifier?: string; type?: string; arrived?: boolean; color?: string }) {
  if (arrived || type === 'arrive') return <Icon name="flag" size={34} color={color}/>;
  const left = modifier?.includes('left');
  const roundabout = type?.includes('roundabout') || type?.includes('rotary');
  return <Svg width={42} height={46} viewBox="0 0 48 48" style={left ? { transform: [{ scaleX: -1 }] } : undefined}>
    {roundabout ? <><Circle cx="24" cy="25" r="12" fill="none" stroke={color} strokeWidth={4}/><Path d="M24 45V37M24 13V3M17 10L24 3L31 10" stroke={color} strokeWidth={4} fill="none" strokeLinecap="round" strokeLinejoin="round"/></> :
      modifier === 'uturn' ? <Path d="M12 43V17C12 3 36 3 36 17V33M29 26L36 33L43 26" stroke={color} strokeWidth={5} fill="none" strokeLinecap="round" strokeLinejoin="round"/> :
      modifier?.includes('left') || modifier?.includes('right') ? <Path d="M12 43V25Q12 15 22 15H41M32 6L41 15L32 24" stroke={color} strokeWidth={5} fill="none" strokeLinecap="round" strokeLinejoin="round"/> :
...
export function DriverNavigation({ navigation, top, onLocation, onHeight }: {
  ...
  const offRoute = (progress?.offRouteMeters || 0) > 60;
  const ready = !!progress && !gpsStatus && !offRoute && !loading && !error;
  const title = gpsStatus || (loading ? 'Строим маршрут' : error ? 'Маршрут недоступен' : offRoute ? 'Вы отклонились от маршрута' : progress?.instruction || 'Готовим навигацию');
  ...
  return <View testID="driver-navigation" pointerEvents="box-none" onLayout={event => onHeight?.(event.nativeEvent.layout.height)} style={[styles.position, { top }]}>
    <View style={styles.guidance}>
      <View style={[styles.turn, theme.isDark && { backgroundColor: theme.palette.accent, shadowColor: '#000000' }]}>{loading ? <ActivityIndicator color={theme.isDark ? theme.palette.accentText : 'white'}/> : <TurnArrow type={turn?.maneuver.type} modifier={turn?.maneuver.modifier} arrived={progress?.arrived} color={theme.isDark ? theme.palette.accentText : 'white'}/>}</View>
