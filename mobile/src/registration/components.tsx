...
import { ActivityIndicator, Image, Modal, Pressable, ScrollView, StyleSheet, Switch, Text, TextInput, View, type ImageStyle } from 'react-native';
...
export function RegistrationHeader({ onBack, onHelp, step, total, title }: { onBack?: () => void; onHelp: () => void; step?: number; total?: number; title?: string }) {
  ...
  return <View style={c.header}>
    <View style={c.headerRow}>
      {onBack ? <Pressable accessibilityRole="button" accessibilityLabel="Назад" hitSlop={12} onPress={onBack} style={c.iconButton}><Icon name="chevron-back" size={24} color={palette.ink}/></Pressable> : <View style={c.iconButton}/>}
...
export function UploadCard({ title, hint, upload, onPress, onDelete, required = true, image, disabled }: { title: string; hint?: string; upload?: RegistrationUpload; onPress: () => void; onDelete?: () => void; required?: boolean; image?: boolean; disabled?: boolean }) {
...
export function ChoiceCard({ title, description, selected, onPress, icon, disabled }: { title: string; description: string; selected: boolean; onPress: () => void; icon: React.ComponentProps<typeof Icon>['name']; disabled?: boolean }) {
  const { palette } = useTheme();
  return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: selected, disabled }} disabled={disabled} onPress={onPress} style={[c.choiceCard, disabled && c.readOnly, { borderColor: selected ? palette.accent : palette.line, backgroundColor: selected ? palette.elevated : palette.surface }]}><View style={[c.choiceIcon, { backgroundColor: selected ? '#DCEEFF' : palette.elevated }]}><Icon name={icon} size={26} color={palette.accent}/></View><View style={{ flex: 1, gap: 4 }}><Text style={[c.choiceTitle, { color: palette.ink }]}>{title}</Text><Text style={[c.choiceDescription, { color: palette.muted }]}>{description}</Text></View><Icon name={selected ? 'checkmark-circle' : 'ellipse-outline'} size={22} color={selected ? palette.accent : palette.line}/></Pressable>;
}
...
const c = StyleSheet.create({
  header: { gap: 12 }, headerRow: { minHeight: 38, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, iconButton: { width: 38, height: 38, alignItems: 'flex-start', justifyContent: 'center' }, helpButton: { minWidth: 66, height: 38, alignItems: 'flex-end', justifyContent: 'center' }, helpText: { fontSize: 14, fontWeight: '600' }, headerTitle: { flex: 1, textAlign: 'center', fontSize: 15, fontWeight: '700' },
  progressRow: { flexDirection: 'row', alignItems: 'center', gap: 10 }, progressTrack: { height: 4, borderRadius: 4, overflow: 'hidden', flex: 1 }, progressFill: { height: 4, borderRadius: 4 }, progressText: { fontSize: 11, fontWeight: '600' },
  primaryButton: { minHeight: 52, borderRadius: 14, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', paddingHorizontal: 18 }, primaryText: { fontSize: 16, fontWeight: '700' }, disabled: { opacity: .42 }, pressed: { opacity: .78 }, secondaryButton: { minHeight: 44, alignItems: 'center', justifyContent: 'center' }, secondaryText: { fontSize: 14, fontWeight: '600' },
  field: { gap: 6 }, label: { fontSize: 11, fontWeight: '600' }, input: { minHeight: 52, borderRadius: 13, borderWidth: 1, paddingHorizontal: 14, fontSize: 15 }, multiline: { minHeight: 104, textAlignVertical: 'top', paddingTop: 14 }, select: { minHeight: 52, borderRadius: 13, borderWidth: 1, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, inputText: { fontSize: 15 }, fieldError: { color: colors.danger, fontSize: 11, lineHeight: 15, marginTop: 2 },
  readOnly: { opacity: .58 },
  ...
  sectionTitle: { gap: 6 }, screenTitle: { fontSize: 27, lineHeight: 33, fontWeight: '800', letterSpacing: -.45 }, screenDescription: { fontSize: 14, lineHeight: 20 }, choiceCard: { minHeight: 92, borderWidth: 1.3, borderRadius: 16, padding: 11, flexDirection: 'row', alignItems: 'center', gap: 11 }, choiceIcon: { width: 54, height: 54, borderRadius: 14, alignItems: 'center', justifyContent: 'center' }, choiceTitle: { fontSize: 14, fontWeight: '700' }, choiceDescription: { fontSize: 11, lineHeight: 15 }, checkboxRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 10 }, checkboxText: { fontSize: 13, lineHeight: 18, flex: 1 }, vehicleCard: { minHeight: 76, borderWidth: 1, borderRadius: 15, padding: 11, flexDirection: 'row', alignItems: 'center', gap: 11 }, vehicleIcon: { width: 52, height: 52, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
});
