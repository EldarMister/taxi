...
import { ActivityIndicator, Image, Modal, Platform, Pressable, ScrollView, StyleSheet, Switch, Text, TextInput, View, type ImageStyle } from 'react-native';
...
export function ChoiceCard({ title, description, selected, onPress, icon, disabled }: { title: string; description: string; selected: boolean; onPress: () => void; icon: React.ComponentProps<typeof Icon>['name']; disabled?: boolean }) {
  const { palette } = useTheme();
  return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: selected, disabled }} disabled={disabled} onPress={onPress} style={({ pressed }) => [c.choiceCard, disabled && c.readOnly, { borderColor: selected ? palette.accent : palette.line, backgroundColor: selected ? '#F4F9FF' : palette.surface }, pressed && c.pressed]}><View style={[c.choiceIcon, { backgroundColor: selected ? '#DCEEFF' : palette.elevated }]}><View style={c.choiceGlow}/><Icon name={icon} size={34} color={palette.accent}/></View><View style={c.choiceCopy}><Text style={[c.choiceTitle, { color: palette.ink }]}>{title}</Text><Text style={[c.choiceDescription, { color: palette.muted }]}>{description}</Text></View><Icon name={selected ? 'checkmark-circle' : 'ellipse-outline'} size={24} color={selected ? palette.accent : palette.line}/></Pressable>;
}
...
const c = StyleSheet.create({
  ...
  uploadCard: { borderWidth: 1, borderRadius: 15, overflow: 'hidden' }, uploadMain: { minHeight: 78, flexDirection: 'row', alignItems: 'center', gap: 11, padding: 11 }, uploadIcon: { width: 52, height: 52, borderRadius: 13, alignItems: 'center', justifyContent: 'center' }, uploadImage: { width: 62, height: 52, borderRadius: 11 }, uploadTitleRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 6 }, uploadTitle: { fontSize: 14, fontWeight: '700', flexShrink: 1 }, uploadHint: { fontSize: 11, lineHeight: 15 }, tinyTrack: { height: 3, borderRadius: 3, overflow: 'hidden' }, tinyFill: { height: 3, borderRadius: 3 }, deleteButton: { position: 'absolute', right: 8, bottom: 5, padding: 5 }, profileUploadCard: { minHeight: 160, borderWidth: 1, borderRadius: 22, overflow: 'hidden' }, profileUploadMain: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 16, padding: 18 }, profilePlaceholder: { width: 112, height: 112, borderRadius: 20, alignItems: 'center', justifyContent: 'flex-end', overflow: 'visible' }, profileImage: { width: 112, height: 112, borderRadius: 20 }, profileHead: { position: 'absolute', top: 23, width: 39, height: 39, borderRadius: 20, backgroundColor: '#BDD0E8' }, profileBody: { width: 72, height: 49, borderTopLeftRadius: 34, borderTopRightRadius: 34, backgroundColor: '#BDD0E8' }, profileAdd: { position: 'absolute', right: -7, bottom: -7, width: 34, height: 34, borderRadius: 17, borderWidth: 3, borderColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' }, profileCopy: { flex: 1, gap: 9 }, profileUploadTitle: { fontSize: 17, fontWeight: '800', flexShrink: 1 }, profileUploadHint: { fontSize: 12, lineHeight: 17 }, profileUploadButton: { minHeight: 38, borderRadius: 11, paddingHorizontal: 11, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, alignSelf: 'flex-start' }, profileUploadButtonText: { fontSize: 12, fontWeight: '800' }, profileDelete: { position: 'absolute', right: 8, bottom: 6, padding: 6 },
  ...
  sectionTitle: { gap: 7 }, screenTitle: { fontSize: 28, lineHeight: 34, fontWeight: '800', letterSpacing: -.5 }, screenDescription: { fontSize: 14, lineHeight: 21 }, choiceCard: { minHeight: 112, borderWidth: 1.4, borderRadius: 20, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 14, elevation: 1, shadowColor: '#17345E', shadowOffset: { width: 0, height: 4 }, shadowOpacity: .06, shadowRadius: 8 }, choiceIcon: { width: 78, height: 78, borderRadius: 18, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }, choiceGlow: { position: 'absolute', width: 72, height: 72, borderRadius: 36, backgroundColor: 'rgba(255,255,255,.55)', transform: [{ translateX: 20 }, { translateY: 24 }] }, choiceCopy: { flex: 1, gap: 5 }, choiceTitle: { fontSize: 16, fontWeight: '800' }, choiceDescription: { fontSize: 12, lineHeight: 17 }, checkboxRow: { minHeight: 44, flexDirection: 'row', alignItems: 'center', gap: 10 }, checkboxText: { fontSize: 13, lineHeight: 18, flex: 1 }, vehicleCard: { minHeight: 76, borderWidth: 1, borderRadius: 15, padding: 11, flexDirection: 'row', alignItems: 'center', gap: 11 }, vehicleIcon: { width: 52, height: 52, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
});
