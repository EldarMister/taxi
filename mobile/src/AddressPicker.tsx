...
export function AddressPicker({ field, center, pickup, dropoff, language, onFieldChange, onSelect, onClose, onMap, onLocation }: {
  ...
  const [query, setQuery] = useState(shortAddress((field === 'pickup' ? pickup : dropoff)?.address));
  ...
  useEffect(() => { setQuery(shortAddress((field === 'pickup' ? pickup : dropoff)?.address)); setEdited(false); setSearch(null); setResults([]); setError(''); if (focusNextField.current) { input.current?.focus(); focusNextField.current = false; } }, [field]);
  useEffect(() => {
  ...
  return <BottomPanel expanded closeRequested={closeRequested} onClose={close} label={t('Закрыть поиск')}>
        <View style={[a.routeCard, isDark && { backgroundColor: palette.elevated, shadowOpacity: 0 }]}>{(['pickup', 'dropoff'] as const).map((item, index) => {
          const active = item === field;
          const point = item === 'pickup' ? pickup : dropoff;
          return <View key={item} style={[a.routeRow, index === 0 && { borderBottomWidth: 1, borderBottomColor: palette.line }]}>
            ...
            <View style={{ flex: 1 }}><Text style={a.caption}>{t(item === 'pickup' ? 'Точка посадки' : 'Пункт назначения')}</Text>{active ? <TextInput ref={input} selectTextOnFocus accessibilityLabel={t('Найти адрес')} testID="address-search" value={query} onChangeText={changeQuery} placeholder={t(item === 'pickup' ? 'Откуда поедем?' : 'Куда едем?')} placeholderTextColor={palette.muted} style={a.input} returnKeyType="search" onSubmitEditing={submitSearch}/> : <Pressable accessibilityRole="button" onPress={() => activateField(item)}><Text numberOfLines={1} style={a.address}>{shortAddress(point?.address) || t(item === 'pickup' ? 'Откуда поедем?' : 'Куда едем?')}</Text></Pressable>}</View>
        ...
        {field === 'pickup' && <Pressable accessibilityRole="button" onPress={() => requestClose(onLocation)} style={a.gps}><Icon name="navigate" color={palette.accent}/><View><Text style={[s.body, isDark && { color: palette.ink }]}>{t('Ваше местоположение')}</Text><Text style={[s.caption, isDark && { color: palette.muted }]}>{t('Определим по GPS')}</Text></View></Pressable>}
        <ScrollView style={a.results} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 12 }} keyboardShouldPersistTaps="handled" keyboardDismissMode="on-drag">
          ...
          {!loading && !waiting && !error && !visibleResults.length && <View style={{ paddingVertical: 24, gap: 8 }}><Text style={a.address}>{t(search?.query === query.trim() ? 'Адрес не найден' : field === 'pickup' ? 'Откуда поедем?' : 'Куда отправимся?')}</Text><Text style={a.caption}>{t(search?.query === query.trim() ? 'Уточните название улицы или выберите точку на карте.' : 'Начните вводить улицу или название места.')}</Text></View>}
        </ScrollView>
  </BottomPanel>;
}
