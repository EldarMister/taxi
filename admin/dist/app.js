...
const sections = { dashboard: ['Обзор', 'grid', 'Сервис сегодня'], orders: ['Заказы', 'orders', 'Все поездки и заказы еды'], drivers: ['Водители', 'drivers', 'Водители, автомобили и баланс'], tariffs: ['Тарифы', 'tariffs', 'Стоимость поездок и комиссия'], restaurants: ['Рестораны', 'food', 'Каталог и настройки заведений'], menu: ['Меню', 'food', 'Категории, блюда и дополнения'], banners: ['Баннеры', 'banner', 'Главный экран приложения'], audit: ['Журнал действий', 'audit', 'Изменения и действия администраторов'] };
...
let kind = 'taxi', page = 1, search = '', filter = '', data = null, loadError = '', busy = false, sequence = 0, editor = null, socket = null, connected = false, toastTimer, liveTimer, filterTimer, dialogReturnFocus;
...
const params = () => new URLSearchParams({ page, pageSize: 25, ...(search ? { search } : {}), ...(route === 'orders' ? { kind, ...(filter ? { status: filter } : {}) } : {}), ...(route === 'drivers' ? { filter: filter || 'all' } : {}) });
...
function renderPage() {
  ...
  const views = { dashboard, orders: () => `<section class="panel">${toolbar(true)}${orderTable(items(data), kind)}${pager(data, busy)}</section>`, drivers: driversView, tariffs: tariffsView, restaurants: restaurantsView, menu: menuView, banners: bannersView, audit: auditView };
...
async function load(quiet = false) {
  ...
  const path = resource === 'dashboard' ? '/admin/dashboard' : `/admin/${resource}${['orders', 'drivers', 'audit'].includes(resource) ? '?' + params() : ''}`;
  busy = true;
  if (!quiet) renderPage();
  try {
...
function dialogBody() { const e = editor; if (e.type === 'tariff') return tariffEditor(e.value); if (e.type === 'driver') return driverEditor(e.value); if (e.type === 'restaurant') return restaurantEditor(e.value, e.isNew); if (e.type === 'banner') return bannerEditor(e.value, restaurants); if (e.type === 'menu-dish') return menuDishEditor(e.value, menuDraft.catalog); if (e.type === 'menu-option') return menuOptionEditor(e.value); if (e.type === 'menu-category') return menuCategoryEditor(e.value.name); if (e.type === 'order') return orderDetail(e.value); if (e.type === 'driver-detail') return driverDetail(e.value); if (e.type === 'topup') return `<div class="form-grid">${field('Сумма пополнения, сом', 'amount', '', { type: 'number', required: true, min: 1, max: 1000000, wide: true })}${field('Комментарий', 'note', '', { required: true, wide: true, maxLength: 250, placeholder: 'Например: наличные в офисе' })}</div>`; if (e.type === 'audit') return `<div class="detail-row"><span>Время</span><strong>${date(e.value.createdAt)}</strong></div><div class="detail-row"><span>Действие</span><strong>${esc(actionNames[e.value.action] || e.value.action)}</strong></div><pre class="audit-details">${esc(JSON.stringify(e.value.details || e.value.metadata || {}, null, 2))}</pre>`; return ''; }
function drawDialog() { if (!editor) return; const readOnly = ['order', 'driver-detail', 'audit'].includes(editor.type); modalRoot.innerHTML = `<div class="modal-shade"><section class="drawer ${editor.type === 'restaurant' ? 'wide' : ''}" role="dialog" aria-modal="true" aria-labelledby="dialog-title"><div class="drawer-head"><div><h2 id="dialog-title">${esc(editor.title)}</h2>${editor.subtitle ? `<p>${esc(editor.subtitle)}</p>` : ''}</div>${button(icon('close'), 'close-dialog', 'aria-label="Закрыть"', 'ghost')}</div><form id="editor-form" style="display:contents"><div class="drawer-body">${dialogBody()}</div><footer class="drawer-footer"><div class="error-inline" role="alert">${esc(editor.error)}</div>${!editor.isNew && ['restaurant', 'banner'].includes(editor.type) ? button(editor.type === 'banner' ? 'Удалить' : 'Скрыть ресторан', 'archive', '', 'danger') : ''}${button(readOnly ? 'Закрыть' : 'Отмена', 'close-dialog')}${readOnly ? '' : `<button class="button primary" type="submit" ${editor.saving ? 'disabled' : ''}>${editor.saving ? 'Сохраняем…' : editor.type === 'topup' ? 'Пополнить' : 'Сохранить'}</button>`}</footer></form></section></div>`; document.body.style.overflow = 'hidden'; if (editor.type === 'menu-dish') updateMenuOptionLimit(modalRoot.querySelector('#editor-form')); const first = modalRoot.querySelector('input:not([type=checkbox]):not([type=file]),button'); first?.focus({ preventScroll: true }); }
