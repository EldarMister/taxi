...
export function tariffEditor(item) {
  return `<div class="form-grid">${field('Название тарифа', 'name', item.name, { required: true, wide: true, maxLength: 80 })}${area('Описание', 'description', item.description).replace('maxlength="1000"', 'maxlength="400"')}${numberField('Посадка, сом', 'basePrice', item.basePrice ?? 50)}${numberField('За километр, сом', 'pricePerKm', item.pricePerKm ?? 15)}${numberField('За минуту, сом', 'pricePerMinute', item.pricePerMinute ?? 0)}${numberField('Минимальная стоимость, сом', 'minimumPrice', item.minimumPrice ?? 50)}${field('Комиссия сервиса, %', 'commissionPercent', (item.commissionBps ?? 1000) / 100, { type: 'number', required: true, min: 0, max: 100, step: '.01' })}<div>${toggle('Тариф доступен клиентам', 'active', item.active ?? true)}</div></div><div class="notice" style="margin-top:24px">Новые цены применяются при расчёте новых поездок. Сумма уже оформленного заказа сохраняется.</div>`;
}

export function driverEditor(item) {
  return `<div class="form-grid">${field('Имя водителя', 'name', item.name, { required: true, wide: true, maxLength: 100 })}${field('Телефон', 'phone', item.phone, { required: true, type: 'tel', placeholder: '+996700123456', wide: true })}</div><div class="form-section"><h3>Автомобиль</h3></div><div class="form-grid">${field('Марка и модель', 'carMake', item.carMake, { required: true, maxLength: 80 })}${field('Цвет', 'carColor', item.carColor, { required: true, maxLength: 40 })}${field('Госномер', 'carPlate', item.carPlate, { required: true, wide: true, maxLength: 20 })}</div><div class="form-section">${toggle('Водитель подтверждён', 'verified', item.verified ?? false, 'После подтверждения водитель сможет принимать заказы.')}</div>`;
}
...
export function readEditor(type, form, item) {
  ...
  if (type === 'tariff') return { ...(item.id ? {} : { id: `tariff-${crypto.randomUUID().slice(0, 8)}` }), name: string('name'), description: string('description'), basePrice: number('basePrice'), pricePerKm: number('pricePerKm'), pricePerMinute: number('pricePerMinute'), minimumPrice: number('minimumPrice'), commissionBps: Math.round(number('commissionPercent') * 100), active: checked('active') };
  if (type === 'driver') return { name: string('name'), phone: string('phone').replace(/[\s()-]/g, ''), carMake: string('carMake'), carColor: string('carColor'), carPlate: string('carPlate'), verified: checked('verified') };
